...
.. code:: python
    ...
    class ValueCountsPrinter(object):
        ...
        def __init__(self, value_counts,
            ...
            :param:
             ...
             - `format_string`: format string for the count/proportion
    column
             - `count_or_proportion`: Header for the count/proportion
    column
            """
    ...
    self.value_counts.index))
        ...
        def row_format_string(self):
            ...
            if self._row_format_string is None:
                self._row_format_string = "{{0:<{0}}}
    {{1:>{1}}}".format(self.first_width,
    self.second_width)
        ...
        def header_string(self):
            ...
            if self._header_string is None:
                self._header_string =
    self.row_format_string.format(self.label,
    self.count_or_proportion)
        ...
        def top_separator(self):
            ...
            if self._top_separator is None:
                self._top_separator = '=' *  (self.first_width +
    self.second_width + 1)
        ...
        def sum_row(self):
            ...
            if self._sum_row is None:
                format_string = '{{0}}
    {{1:>{0}}}'.format(self.second_width)
                sum_value =
    self.format_string.format(self.value_counts.values.sum())
                self._sum_row = format_string.format(' ' *
    self.first_width,
                                                     sum_value)
    ...
    self.format_string.format(self.value_counts.values[index]))
                                 for index,value in
    enumerate(self.value_counts.index)))
...
.. code:: python
    ...
    class ValueProportionsPrinter(ValueCountsPrinter):
        ...
        def __init__(self, value_counts, label,
            ...
            super(ValueProportionsPrinter,
    self).__init__(value_counts=value_counts,
                                                          label=label,
...
I think this was replaced by the `ValueCountsPrinter` above.
...
.. code:: python
    ...
    def print_value_counts(value_counts, header, format_string='{0}'):
        ...
        if value_counts.index.dtype == 'object':
            first_width = max(first_width, max(len(i) for i in
    value_counts.index))
        second_width = len(header[1])
        format_string = "{{0:<{0}}} {{1:>{1}}}".format(first_width,
    second_width)
...
This was created to summarize the properties for a value in a data set to make creating a code-book easier.


.. code:: python

    def print_properties(data_type, values, construction, missing='None',
    table_format='orgtbl'):
        """
        ...
        print(tabulate([['Data Type', data_type],
                        ...
                        ['Construction', "Created from
    '{0}'".format(construction)]],
